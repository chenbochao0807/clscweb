...
export async function GET() {
	...
	for (const tag of tags) {
		entries.push(
			urlEntry(
				absolute(`/tags/${encodeURIComponent(tag.name)}/`),
				now,
			),
		);
	}
	...
	for (const category of categories) {
		...
		entries.push(
			urlEntry(
				absolute(`/categories/${encodeURIComponent(category.name)}/`),
				now,
			),
		);
	}
	...
	entries.push(
		urlEntry(absolute("/categories/uncategorized/"), now),
	);
	...
	for (const achievement of achievements) {
		if (achievement.data.photo) {
			achievementImages.push({
				...
				caption: String(achievement.data.year),
			});
		}
	}
